...
usage() {
  cat <<'EOF' >&2
Usage: train_wd_dists.sh [base_data_dir] [options]
...
if [ -z "${TRAIN_ROOT}" ]; then
  if [ -n "${BASE_ROOT}" ]; then
    TRAIN_ROOT="${BASE_ROOT%/}/open_images"
  else
    TRAIN_ROOT="${PROJECT_ROOT}/data/open_images"
  fi
fi
