"""
Phase 2 WD+DISTS Training: Wasserstein Distortion + DISTS.

Loss = λ_rd × MSE + λ_wd × WD + λ_dists × DISTS + λ_style × Style + λ_gan × GAN + λ_bpp × BPP

LPIPS is computed during validation for comparison only.
"""
...
from utils.training import train_one_epoch_gan_wd
from utils.testing import test_one_epoch_gan_wd
from loss.rd_loss import RateDistortionPOELICLossWDDISTS
...
def main():
    ...
    criterion = RateDistortionPOELICLossWDDISTS(lmbda=args.lmbda, device=device, gpu_id=args.gpu_id, metrics='mse')
    ...
    logger_train.info("PHASE 2 OURS: WD + DISTS Method")
    logger_train.info("Loss = λ_rd × MSE + λ_wd × WD + λ_dists × DISTS + λ_style × Style + λ_gan × GAN + λ_bpp × BPP")
    logger_train.info("=" * 80)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        current_step = train_one_epoch_gan_wd(
            net,
            net_disc,
            criterion,
            train_dataloader,
        ...
        loss = test_one_epoch_gan_wd(epoch, test_dataloader, net, net_disc, criterion, save_dir, logger_val, tb_logger, config)
        lr_scheduler.step()
        lr_scheduler_D.step()
