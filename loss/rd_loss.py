...
try:
    from piq import DISTS, PieAPP  # Both perceptual metrics enabled
    PIQ_AVAILABLE = True
    PIEAPP_AVAILABLE = True
except ImportError:
    print("Warning: piq library not found. Install with: pip install piq")
    PIQ_AVAILABLE = False
    PIEAPP_AVAILABLE = False
...
class RateDistortionPOELICLossPhase2(nn.Module):
    ...
    Adds PIEAPP for structure-aware perceptual quality (patch-based, rate-efficient).
    """
    ...
    def __init__(self, lmbda=1e-2, device="cuda", gpu_id=None, metrics='mse'):
        ...
        # Initialize DISTS for enhanced perceptual loss (texture-aware)
        if PIQ_AVAILABLE:
            self.dists = DISTS().to(device)
        else:
            self.dists = None
            print("Warning: DISTS not available. Install piq library for enhanced perceptual loss.")
        
        # Initialize PIEAPP for structure-aware perceptual loss (patch-based)
        if PIEAPP_AVAILABLE:
            self.pieapp = PieAPP().to(device)
        else:
            self.pieapp = None
            print("Warning: PIEAPP not available. Install piq library for structure-aware perceptual loss.")
    ...
    def forward(self, output, target):
        ...
        # Compute PIEAPP loss (structure-aware perceptual loss)
